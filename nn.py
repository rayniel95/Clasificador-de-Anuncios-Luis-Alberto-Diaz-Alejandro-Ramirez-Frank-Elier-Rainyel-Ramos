...
def cnn(input_size, vocab_size, MAX_SEQUENCE_LENGTH, pretrained_weights = None):
	...
	convs = []
	for fsz in [2,3,4,5]:
		conv = Conv1D(32, fsz, activation='relu')(concat1)
		pool = MaxPooling1D(5)(conv)
		pool = Dropout(0.3)(pool)
		convs.append(pool)
	concat2 = Concatenate(axis=1)(convs)

	convs = []
	for fsz in [2,3,4,5]:
		conv = Conv1D(16, fsz, activation='relu')(concat2)
		pool = MaxPooling1D(5)(conv)
		pool = Dropout(0.3)(pool)
		convs.append(pool)
	concat3 = Concatenate(axis=1)(convs)

	flat1 = Flatten()(concat3)
...
def rnn(input_size, vocab_size, MAX_SEQUENCE_LENGTH, pretrained_weights = None):
	inputs = Input(input_size)
	embb1 = Embedding(vocab_size, 32, input_length = MAX_SEQUENCE_LENGTH)(inputs)
	bidir1 = Bidirectional(LSTM(64, return_sequences=True))(embb1)
	bidir2 = Bidirectional(LSTM(32))(bidir1)
	dense1 = Dense(50, activation='relu')(bidir2)
	dense2 = Dense(41, activation='softmax')(dense1)
...
if __name__ == '__main__':
	...
	model = cnn((ms,), len(v) + 1, ms)

	gen = Generator('/home/luis/Desktop/SI/jrev', v, c, ms, BATCHSIZE)
